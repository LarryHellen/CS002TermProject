...
void featureSelection()
{
	...
	*/
	...
	optionList currentFeatures[] = {dateSelection, taskCategoryMenu, return};
	string optionNames[] = { DATE_SELECT, CATEGORY_MENU, EXIT };

	if (!(currentDate[0] == 0 && currentDate[1] == 0 && currentDate[2] == 0))
	{
		currentFeatures = { dateSelection, taskCategoryMenu, taskCreation, taskDeletion, taskView, return };
		optionNames[] = { DATE_SELECT, CATEGORY_MENU, NEW_TASK, DELETE_TASK, VIEW_TASK, EXIT };
	}
	...
	*/
	...
	menu mainMenu;
	mainMenu.syncOptions(currentFeatures, optionNames);
...
void taskCreation()
{
	...
	*/
	...
	fromTaskCreation.syncOptions(currentFeatures, optionNames);
...
void taskDeletion()
{
	...
	*/
	...
	fromTaskDeletion.syncOptions(currentFeatures, optionNames);
...
void dateSelection()
{
	...
	*/
	...
	menu fromSetDate;
	fromSetDate.syncOptions(currentFeatures, optionNames);
...
void taskCategoryMenu()
{
	...
	*/
	...
	menu fromCatMenu;
	fromCatMenu.syncOptions(currentFeatures, optionNames);
...
void taskCategoryCreator()
{
	...
	*/
	...
	menu fromCatCreate;
	fromCatCreate.syncOptions(currentFeatures, optionNames);
...
void taskCategoryDeleter()
{
	...
	*/
	...
	menu fromCatDelete;
	fromCatDelete.syncOptions(currentFeatures, optionNames);
...
public:
	optionList options[];
	std::string optionTitles[];
	int choice;


	void syncOptions(optionList list[], std::string messages[])
	{
		options = new optionList[sizeof(list)];
		optionTitles = new std::string[sizeof(messages)]
	...
	void menuChoice()
	{
		...
		while (true)
		{
			...
			if (invalid || choice > (sizeof(options) + 1) || choice < 1 || choice != floor(choice))
			{
